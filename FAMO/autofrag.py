#!/home/zhangjx/ssd/softwares/anaconda3/bin/python
import os
import sys
import re
import numpy as np
...
def parse_id(fnid):
    ...
    for l in text:
        ...
        charges.append(list(map(int, ll.pop(0).split('_'))) if ll else [0])
        # muls.append(int(ll.pop(0)) if ll else 0)
        muls.append(list(map(int, ll.pop(0).split('_'))) if ll else [1])
...
def gen_ofns(fragments, charges, labels):
    ...
    for n in tmp:
        if c[n] > 1:
            ...
            names.append(n)
    return ['%s-%s' % (name, ('p' + str(c[0])) if (c[0] >= 0) else ('n' + str(-c[0]))) 
...
def main():
    ...
    for f, c, m in zip(fragmentation, charges, muls):
        ...
        frag = Molecule(atoms, charge=c[0], multiplicity=abs(m[0]))
    ...
    for fragid, frag in enumerate(fragments):
        if links[fragid]:
            continue
        links[fragid] = frag.title+'.47' # for NBO6 or newer
    ...
    with open(fnid+'.rev', 'w') as f:
        for fr, c, m, n, l in zip(fragmentation, charges, muls, labels, links):
            f.write('%s %s %s %s %s\n' % 
                (fr, '_'.join('%d' % _ for _ in c), '_'.join('%d' % _ for _ in m), n, l))
        f.close()
